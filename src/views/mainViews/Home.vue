<template>
  <div class="home">
    <van-nav-bar :fixed="true" title="我的书架" right-text="书城" left-text="返回" left-arrow>
      
    </van-nav-bar>
  
    <ul class="fav-books">
      <li class="book-card-item" @click="clickBookCard(item)" v-for="item in bookSelft" :key=item._id>
        <img
          class="book-card-img"
          :src="item.cover"
          alt
      ...
      <!-- <li class="book-card-item">
        <img
          class="book-card-img"
          src="http://ww4.sinaimg.cn/mw690/6a1f03d3gw1e6qh5p99zoj205k07s0tc.jpg"
          alt
        />
        <div class="book-detail">
          <h5>元尊</h5>
          <span class="book-auto">天蚕土豆</span>
          <span class="book-class">玄幻</span>
          <p>彼时的归途，已是一条命运倒悬的路</p>
          <span class="book-num">284万字</span>
          <span class="book-old">64%留存率</span>
        </div>
      </li>
      <li class="book-card-item">
        <img
    ...
    clickBookCard(item){
      ...
      this.axios({
        ...
        .then((result) => {
          ...
          this.$router.push({name: 'Reader',query: {link,shuYuanId:item.shuYuanId,index:chapterIndex,title}});
        })
        .catch((err) => {
          this.$toast.clear();
        });
      // localStorage.setItem("weibooks",'哈哈哈')
    },
    // 删除一个书
    removeBooks(shuId){
