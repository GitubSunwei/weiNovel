...
export default {
  ...
  methods: {
    ...
    goList(){
      ...
      this.$router.push({name:'CatalogList',query:{shuYuanId,bookName: this.bookDetail.title}})
    },

    // 加入书架
    addBookshelf(){
      let {cover,title,author,majorCate,longIntro,wordCount,retentionRatio} = this.bookDetail;
      let chapterIndex = 0;
      let pageNum = 0;
      if(localStorage.getItem("read_chapter_"+this.shuYuanId)){
        chapterIndex = localStorage.getItem("read_chapter_"+this.shuYuanId);
      }
      let obj = {shuYuanId: this.shuYuanId,chapterIndex,pageNum,cover,title,author,majorCate,longIntro,wordCount,retentionRatio};
    ...
    startRead(){
      ...
      this.axios({
        ...
        .then((result) => {
          ...
          this.$router.push({name: 'Reader',query: {link,shuYuanId,index,title}});
        })
        .catch((err) => {
          this.$toast.clear();
        });
